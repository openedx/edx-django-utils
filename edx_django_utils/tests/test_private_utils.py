...
class TestPrivateUtils(TestCase):  # pylint: disable=missing-class-docstring
    ...
    @unittest.skipIf(
        django.VERSION[0] > 1,
        "MIDDLEWARE_CLASSES only works with less than django-2 versions.",
    )
    ...
    def test_check_middleware_dependencies_complex_success(self):
        ...
        _check_middleware_dependencies(
            middleware,
            required_middleware=["required.2.Middleware", "required.4.Middleware",],
        )
    ...
    def test_check_middleware_dependencies_bad_order(self):
        ...
        with self.assertRaises(AssertionError):
            _check_middleware_dependencies(
                middleware,
                required_middleware=["required.1.Middleware", "required.2.Middleware",],
            )
