...
class TestRequestCacheMiddleware(TestCase):  # pylint: disable=missing-class-docstring
    @override_settings(
        MIDDLEWARE=["edx_django_utils.cache.middleware.RequestCacheMiddleware"]
    )
    def setUp(self):
...
class TestTieredCacheMiddleware(TestCase):  # pylint: disable=missing-class-docstring
    ...
    def test_process_request(self):
        ...
        self.assertFalse(
            self.request_cache.get_cached_response(SHOULD_FORCE_CACHE_MISS_KEY).value
        )
