...
class TestRequestCache(TestCase):  # pylint: disable=missing-class-docstring
    ...
    def test_cache_data(self):
        ...
        key_value_pairs = [(TEST_KEY, EXPECTED_VALUE), (TEST_KEY_2, EXPECTED_VALUE_2)]
        expected_dict = {}
    ...
    def test_clear_all_namespaces_other_thread(self):
        ...
        self.request_cache.set(TEST_KEY, EXPECTED_VALUE)
        other_thread = Thread(
            target=lambda: RequestCache.clear_all_namespaces()
...
class TestTieredCache(TestCase):  # pylint: disable=missing-class-docstring
    ...
    def test_set_all_tiers(self, mock_cache_set):
        ...
        mock_cache_set.assert_called_with(
            TEST_KEY, EXPECTED_VALUE, TEST_DJANGO_TIMEOUT_CACHE
        )
        self.assertEqual(
    ...
    def test_delete(self, mock_cache_delete):
        ...
        self.assertFalse(self.request_cache.get_cached_response(TEST_KEY).is_found)
        self.assertEqual(
            self.request_cache.get_cached_response(TEST_KEY_2).value, EXPECTED_VALUE
        )
...
class CacheResponseTests(TestCase):  # pylint: disable=missing-class-docstring
    def test_is_miss(self):
        ...
        self.assertEqual(
            cached_response.get_value_or_default(EXPECTED_VALUE_2), EXPECTED_VALUE_2
        )
        self.assertIn(
            "CachedResponse(is_found={}, key={}".format(False, TEST_KEY),
            cached_response.__repr__(),
        )
    ...
    def test_is_hit(self):
        ...
        self.assertEqual(cached_response.value, EXPECTED_VALUE)
        self.assertEqual(
            cached_response.get_value_or_default(EXPECTED_VALUE_2), EXPECTED_VALUE
        )
        self.assertIn(
...
class TestCacheUtils(TestCase):
    ...
    @ddt.data(
        ...
        (
            get_cache_key(resource="catalogs", site_domain="example.com"),
            True,
        ),  # test sorted arguments
        (
            get_cache_key(site_domain=b"example.com", resource="catalogs"),
            True,
        ),  # test byte str value
        (
            get_cache_key(site_domain="example.com", resource="users"),
            False,
        ),  # test one different argument
        (
            get_cache_key(site_domain="other.com", resource="catalogs"),
            False,
        ),  # test another different argument
    )
    @ddt.unpack
