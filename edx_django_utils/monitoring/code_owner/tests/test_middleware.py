...
class CodeOwnerMetricMiddlewareTests(TestCase):
    ...
    @ddt.data(
        ("/middleware-test/", None), ("/test/", "team-red"),
    )
    @ddt.unpack
    def test_code_owner_path_mapping_hits_and_misses(
        ...
        with patch(
            ...
            self._assert_code_owner_custom_metrics(
                ...
                path_module=expected_path_module,
            )
    ...
    @patch("edx_django_utils.monitoring.code_owner.middleware.set_custom_metric")
    @ddt.data(
        ("/middleware-test/", "team-blue"), ("/test/", "team-red"),
    )
    @ddt.unpack
    ...
    @patch("edx_django_utils.monitoring.code_owner.middleware.set_custom_metric")
    @patch("newrelic.agent")
    def test_code_owner_transaction_mapping_error(
        self, mock_newrelic_agent, mock_set_custom_metric
    ):
        mock_newrelic_agent.current_transaction = Mock(
    ...
    @override_settings(
        CODE_OWNER_MAPPINGS={
            "team-red": ["lms.djangoapps.monitoring.tests.mock_views"]
        },
    )
    @patch("edx_django_utils.monitoring.code_owner.middleware.set_custom_metric")
    ...
    @override_settings(CODE_OWNER_MAPPINGS={"team-red": ["*"]},)
    @patch("edx_django_utils.monitoring.code_owner.middleware.set_custom_metric")
    def test_catch_all_instead_of_errors(self, mock_set_custom_metric):
        with patch(
            "edx_django_utils.monitoring.code_owner.utils._PATH_TO_CODE_OWNER_MAPPINGS",
            _process_code_owner_mappings(),
        ):
            ...
            self.middleware(request)
            self._assert_code_owner_custom_metrics(
                mock_set_custom_metric, expected_code_owner="team-red"
            )

    @override_settings(
        CODE_OWNER_MAPPINGS=["invalid_setting_as_list"], ROOT_URLCONF=__name__,
    )
    @patch("edx_django_utils.monitoring.code_owner.middleware.set_custom_metric")
    ...
    def _assert_code_owner_custom_metrics(
        ...
        self.assertEqual(
            len(mock_set_custom_metric.call_args_list),
            len(call_list),
            "Expected calls {} vs actual calls {}".format(
                call_list, mock_set_custom_metric.call_args_list
            ),
        )
