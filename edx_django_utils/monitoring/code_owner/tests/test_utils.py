...
class MonitoringUtilsTests(TestCase):
    ...
    @override_settings(
        CODE_OWNER_MAPPINGS={"team-red": ["openedx.core.djangoapps.xblock"]}
    )
    def test_is_config_loaded_with_valid_dict(self):
        with patch(
            ...
            self.assertTrue(
                is_code_owner_mappings_configured(), "Mappings should be configured."
            )
    ...
    @override_settings(
        CODE_OWNER_MAPPINGS={
            "team-red": ["openedx.core.djangoapps.xblock", "lms.djangoapps.grades",],
        ...
        with override_settings(CODE_OWNER_MAPPINGS=code_owner_mappings):
            with patch(
                ...
                self.assertLess(
                    average_time,
                    0.0005,
                    "Mapping takes {}s which is too slow.".format(average_time),
                )
