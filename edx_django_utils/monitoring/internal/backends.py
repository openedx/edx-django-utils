...
class OpenTelemetryBackend(TelemetryBackend):
    ...
    OpenTelemetry will need to be configured and initialized separately.
    """
    def set_attribute(self, key, value):
    ...
    def record_exception(self):
        get_current_span().record_exception(sys.exc_info()[1])
...
def configured_backends():
    ...
    #   such as the built-in `edx_django_utils.monitoring.NewRelicBackend` and
    #   `edx_django_utils.monitoring.OpenTelemetryBackend`. For historical reasons, this defaults to just
